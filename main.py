...
<style>
    ...
    .timeline-item.done { border-left-color: #2e7d32; background: #eef8ef; }
    .timeline-item.current { border-left-color: #1565c0; background: #eef5ff; }
    .timeline-item.pending { border-left-color: #b0bec5; background: #fafafa; }
    .tracking-protocol-box {
        background: #0f172a;
        color: #ffffff;
        border-radius: 14px;
        padding: 16px 18px;
    ...
    try:
        ...
        cur.execute("""
            ...
            BEGIN
                ...
                END IF;
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='notifications' AND column_name='public_tracking_code') THEN
                    ALTER TABLE notifications ADD COLUMN public_tracking_code VARCHAR(40);
                END IF;
            END
            $$;

            CREATE UNIQUE INDEX IF NOT EXISTS idx_notifications_public_tracking_code
                ON notifications (public_tracking_code)
...
def load_notifications() -> List[Dict]:
    ...
    try:
        ...
        cur.execute("""
            SELECT
                id, public_tracking_code, title, description, location, occurrence_date, occurrence_time,
...
def create_notification(data: Dict, uploaded_files: Optional[List[Any]] = None):
    ...
    try:
        ...
        public_tracking_code = generate_public_tracking_code(conn)
        ...
        add_history_entry(
            ...
            f"Notificação enviada para classificação. Protocolo: {public_tracking_code}. Título: {data.get('title', 'Sem título')[:100]}..." if len(data.get('title','')) > 100
              else f"Notificação enviada para classificação. Protocolo: {public_tracking_code}. Título: {data.get('title', 'Sem título')}",
            conn=conn,
            cursor=cur
        )
...
def update_notification(notification_id: int, updates: Dict):
    ...
    try:
        ...
        for key, value in updates.items():
            if key in ['id', 'created_at', 'attachments', 'actions', 'history', 'updated_at', 'public_tracking_code']:
                continue
...
PUBLIC_STATUS_LABELS = {
    'pendente_classificacao': 'Aguardando classificação inicial',
    'aguardando_classificador': 'Em revisão pelo classificador',
    'revisao_classificador_execucao': 'Em revisão da execução',
    'classificada': 'Classificada',
    'classificada_aguardando_execucao': 'Classificada e aguardando execução',
...
PUBLIC_TIMELINE_STEPS = [
    ('abertura', 'Notificação registrada'),
    ('classificacao', 'Classificação inicial'),
    ('execucao', 'Tratativa / execução'),
...
def format_tracking_code_for_display(code: str) -> str:
    norm = normalize_tracking_code(code)
    if len(norm) >= 18 and norm.startswith('NS'):
        return f"{norm[:2]}-{norm[2:10]}-{norm[10:15]}-{norm[15:]}"
    return (code or '').strip().upper()

def generate_public_tracking_code(conn, max_attempts: int = 15) -> str:
    cur = conn.cursor()
    try:
        for _ in range(max_attempts):
            rand = ''.join(secrets.choice(TRACKING_ALPHABET) for _ in range(12))
            raw_code = f"NS{datetime.now().strftime('%Y%m%d')}{rand}"
            cur.execute(
                "SELECT 1 FROM notifications WHERE public_tracking_code = %s LIMIT 1",
                (raw_code,)
            )
        ...
        cur.close()
    raise RuntimeError('Não foi possível gerar um protocolo público único.')
...
def mask_sensitive_value(value: Any, keep_last: int = 4) -> str:
...
def get_notification_by_tracking_code(tracking_code: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        notification = dict(zip(cols, row))
        cur.close()

        for key in ('occurrence_date', 'occurrence_time', 'created_at', 'updated_at'):
            if notification.get(key) and hasattr(notification[key], 'isoformat'):
                notification[key] = notification[key].isoformat()

        nid = int(notification['id'])
        notification['attachments'] = _get_attachments_map_by_ids(conn, [nid]).get(nid, [])
        notification['history'] = _get_history_map_by_ids(conn, [nid]).get(nid, [])
        notification['actions'] = _get_actions_map_by_ids(conn, [nid]).get(nid, [])
        return notification
    except Exception as e:
        st.error(f"Erro ao consultar protocolo: {e}")
        return None
    finally:
        if conn:
            conn.close()
...
def build_public_timeline(notification: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for idx, (step_key, label) in enumerate(steps):
        ...
        if idx < current_index:
            ...
            state = 'current'
        if step_key == 'abertura':
            state = 'done'
        timestamp = stage_times.get(step_key)
        timeline.append({
            'step': step_key,
            'label': label,
            'state': state,
...
def render_public_notification_summary(notification: Dict[str, Any]):
    classif = _safe_json_dict(notification.get('classification'))
    approval = _safe_json_dict(notification.get('approval'))
    rejection_approval = _safe_json_dict(notification.get('rejection_approval'))
    conclusion = _safe_json_dict(notification.get('conclusion'))

    st.markdown("### Resumo da notificação")
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("<div class='public-summary-card'>", unsafe_allow_html=True)
        st.write(f"**Protocolo:** {format_tracking_code_for_display(notification.get('public_tracking_code'))}")
        st.write(f"**Título:** {notification.get('title', UI_TEXTS.text_na)}")
        st.write(f"**Status atual:** {get_public_status_label(notification.get('status'))}")
        st.write(f"**Local:** {notification.get('location', UI_TEXTS.text_na)}")
        st.write(f"**Data/Hora da ocorrência:** {format_date_time_summary(notification.get('occurrence_date'), notification.get('occurrence_time'))}")
        ...
        st.write(f"**Setor notificante:** {reporting_department}")
        st.write(f"**Setor notificado:** {notified_department}")
        st.write(f"**Paciente envolvido:** {'Sim' if truthy(notification.get('patient_involved')) else 'Não'}")
        if truthy(notification.get('patient_involved')) and notification.get('patient_id'):
            st.write(f"**Prontuário/atendimento:** {mask_sensitive_value(notification.get('patient_id'))}")
        st.markdown("</div>", unsafe_allow_html=True)
    with col2:
        st.markdown("<div class='public-summary-card'>", unsafe_allow_html=True)
        st.write(f"**Classificação NNC:** {classif.get('nnc', UI_TEXTS.text_na)}")
        if classif.get('nivel_dano'):
            st.write(f"**Nível de dano:** {classif.get('nivel_dano')}")
        if classif.get('prioridade'):
            st.write(f"**Prioridade:** {classif.get('prioridade')}")
        if classif.get('deadline'):
            try:
                st.write(f"**Prazo definido:** {datetime.fromisoformat(classif.get('deadline')).strftime('%d/%m/%Y')}")
            except Exception:
                st.write(f"**Prazo definido:** {classif.get('deadline')}")
        if approval.get('decision'):
            st.write(f"**Resultado da aprovação:** {approval.get('decision')}")
        elif rejection_approval.get('decision'):
            st.write(f"**Resultado da aprovação:** {rejection_approval.get('decision')}")
        if conclusion.get('status_final'):
            st.write(f"**Status final registrado:** {str(conclusion.get('status_final')).replace('_', ' ').title()}")
        if conclusion.get('timestamp'):
            try:
                st.write(f"**Encerramento em:** {datetime.fromisoformat(conclusion.get('timestamp')).strftime('%d/%m/%Y %H:%M:%S')}")
            except Exception:
                st.write(f"**Encerramento em:** {conclusion.get('timestamp')}")
        st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div class='public-summary-card'>", unsafe_allow_html=True)
    st.markdown("**Descrição do evento**")
    st.info(notification.get('description', UI_TEXTS.text_na))
    if notification.get('additional_notes'):
        st.markdown("**Observações adicionais do notificante**")
        st.write(notification.get('additional_notes'))
...
def show_tracking_page():
    st.markdown("<h1 class='main-header'>🔎 Acompanhar notificação</h1>", unsafe_allow_html=True)
    st.markdown("<div class='home-modal-card'>", unsafe_allow_html=True)
    st.write("Informe o protocolo gerado no envio para acompanhar a situação da notificação.")
    default_value = st.session_state.get('tracking_code_input', '')
    entered_code = st.text_input(
        "Protocolo da notificação",
        value=default_value,
        placeholder="Ex.: NS-20260328-ABCDE-FGHJKLMN",
        key='tracking_code_input'
    )

    col_a, col_b = st.columns([1, 1])
    with col_a:
        if st.button("🔎 Consultar protocolo", use_container_width=True, key='tracking_lookup_btn'):
            st.session_state.tracking_lookup_requested = True
    with col_b:
        if st.button("📝 Nova notificação", use_container_width=True, key='tracking_go_create_btn'):
            st.session_state.page = 'home'
            _reset_form_state()
            st.rerun()

    lookup_requested = st.session_state.get('tracking_lookup_requested', False)
    if lookup_requested or entered_code:
        notification = get_notification_by_tracking_code(entered_code)
        if notification:
            st.success(f"Protocolo localizado: {format_tracking_code_for_display(notification.get('public_tracking_code'))}")
            st.markdown(f"**Status atual:** {get_public_status_label(notification.get('status'))}")
            st.markdown("### Linha do tempo")
            for item in build_public_timeline(notification):
                ts = item.get('timestamp')
                ts_text = 'Ainda não registrado'
                if ts:
                    try:
                        ts_text = datetime.fromisoformat(str(ts)).strftime('%d/%m/%Y %H:%M:%S')
                    except Exception:
                        ts_text = str(ts)
                icon = '✅' if item['state'] == 'done' else '🟦' if item['state'] == 'current' else '⏳'
                state_label = 'Concluída' if item['state'] == 'done' else 'Etapa atual' if item['state'] == 'current' else 'Pendente'
                st.markdown(
                    f"<div class='timeline-item {item['state']}'><strong>{icon} {item['label']}</strong><br><small>{state_label} • {ts_text}</small></div>",
                    unsafe_allow_html=True
                )
            render_public_notification_summary(notification)
        elif entered_code.strip():
            st.error("Nenhuma notificação foi encontrada para o protocolo informado. Revise o código e tente novamente.")
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        if st.session_state.authenticated and st.session_state.user:
            ...
            if st.button("📝 Nova Notificação", key="nav_create_notif", use_container_width=True):
                ...
                if 'approval_form_state' in st.session_state: st.session_state.pop('approval_form_state')
                st.rerun()
            if st.button("🔎 Acompanhar Notificação", key="nav_tracking", use_container_width=True):
                st.session_state.page = 'tracking'
                st.rerun()
...
def main():
    ...
    if 'tracking_code_input' not in st.session_state: st.session_state.tracking_code_input = ''
    if 'tracking_lookup_requested' not in st.session_state: st.session_state.tracking_lookup_requested = False
    if 'last_created_notification' not in st.session_state: st.session_state.last_created_notification = None
    ...
    if st.session_state.page == 'home':
        show_home_page()
    elif st.session_state.page == 'create_notification':
