...
def show_execution():
    ...
    NOVO LAYOUT (2026-01): duas guias:
      - Pendentes de Execução
      - Executadas (minhas execuções concluídas)
    Ambas exibem cards colapsados (expander) com: nº, título, data de criação, prazo e status.
    """
    ...
    st.info("Use as guias abaixo: **Pendentes de Execução** (fluxo de execução) e **Executadas** (somente leitura + suas ações).")
    ...
    def _safe_parse_classification(n: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _fmt_dt(iso_str: str, fmt: str = "%d/%m/%Y %H:%M") -> str:
        if not iso_str:
    ...
    def _executor_concluded(n: Dict[str, Any], uid: int) -> bool:
        """Retorna True se ESTE executor já marcou sua parte como concluída (final_action_by_executor=True)."""
        # Preferir actions já carregadas (otimizado); fallback para query dedicada se vier vazio.
        actions = n.get('actions') or []
        if not actions:
            try:
                actions = get_notification_actions(n.get('id'))
            except Exception:
                actions = []
        for a in actions:
            if a.get('executor_id') == uid and bool(a.get('final_action_by_executor')) is True:
                return True
        return False

    # Filtragem principal
    assigned_to_me = [n for n in all_notifications if user_id_logged_in in (n.get('executors') or [])]

    pending_notifications = []
    executed_notifications = []

    for n in assigned_to_me:
        concluded_by_me = _executor_concluded(n, user_id_logged_in)
        if concluded_by_me:
            executed_notifications.append(n)
        elif n.get('status') in active_execution_statuses:
            pending_notifications.append(n)

    # Ordenação (prioridade + mais antigas primeiro dentro da prioridade)
    priority_order = {p: i for i, p in enumerate(FORM_DATA.prioridades)}
    def _sort_key(n: Dict[str, Any]):
        c = _safe_parse_classification(n)
        pr = c.get('prioridade', UI_TEXTS.text_na)
        pr_idx = priority_order.get(pr, len(FORM_DATA.prioridades))
        created = n.get('created_at') or "1900-01-01T00:00:00"
        try:
            created_ts = datetime.fromisoformat(created.replace("Z", "+00:00")).timestamp()
        except Exception:
            created_ts = 0
        return (pr_idx, created_ts)

    pending_notifications.sort(key=_sort_key)
    # Executadas: mais recentes primeiro
    executed_notifications.sort(key=lambda x: (x.get('created_at') or ""), reverse=True)

    tab_pending, tab_executed = st.tabs([
        f"⏳ Pendentes de Execução ({len(pending_notifications)})",
        f"✅ Executadas ({len(executed_notifications)})"
    ])

    def _render_actions_history(notification: Dict[str, Any], only_mine: bool = False):
        actions = notification.get('actions') or []
        if not actions:
            actions = get_notification_actions(notification.get('id'))

        if only_mine:
            actions = [a for a in actions if a.get('executor_id') == user_id_logged_in]
        ...
        for action in sorted_actions:
            ...
            is_mine = (user_id_logged_in and action.get('executor_id') == user_id_logged_in)
            card_cls = "my-action-entry-card" if is_mine else "action-entry-card"
            who = f"VOCÊ ({action.get('executor_name', UI_TEXTS.text_na)})" if is_mine else action.get('executor_name', UI_TEXTS.text_na)

            st.markdown(f"""
            <div class='{card_cls}'>
                <strong>{action_type}</strong> - por <strong>{who}</strong> em {action_timestamp}
                <br>
                <em>{action.get('description', UI_TEXTS.text_na)}</em>
            </div>
            """, unsafe_allow_html=True)

            # Evidências (apenas na ação final)
            if action.get('final_action_by_executor'):
    ...
    def _render_collapsed_cards(notifs: List[Dict[str, Any]], mode: str):
        """
        mode:
          - 'pending': mostra detalhes + fluxo de execução (registrar ação / concluir)
          - 'executed': somente leitura + minhas ações
        """
        if not notifs:
        ...
        for notification in notifs:
            classif_info = _safe_parse_classification(notification)
            ...
            created_at = _fmt_dt(notification.get('created_at'), "%d/%m/%Y %H:%M")
            status_text = (notification.get('status', UI_TEXTS.text_na) or UI_TEXTS.text_na).replace('_', ' ').title()

            title = notification.get('title', UI_TEXTS.text_na)
            label = f"#{notification.get('id', UI_TEXTS.text_na)} | {title} | Criada: {created_at} | Prazo: {deadline_status['text']} | Status: {status_text}"
            ...
            with st.expander(label, expanded=False):
                ...
                st.markdown("#### 🧾 Histórico de Ações")
                if mode == "executed":
                    st.caption("Mostrando **apenas suas ações** nesta notificação.")
                    _render_actions_history(notification, only_mine=True)
                else:
                    _render_actions_history(notification, only_mine=False)

                # Fluxo de execução (apenas pendentes)
                if mode != "pending":
                    continue
                ...
                action_choice_key = f"exec_action_choice_{notification.get('id', UI_TEXTS.text_na)}_v2tabs"
                ...
                action_type_choice_options = [UI_TEXTS.selectbox_default_acao_realizar, "Registrar Ação", "Concluir Minha Parte"]
                st.selectbox(
                    "Qual ação deseja realizar?*",
                    options=action_type_choice_options,
                ...
                with st.form(f"action_form_{notification.get('id', UI_TEXTS.text_na)}_v2tabs", clear_on_submit=False):
                    action_description_state = st.text_area(
                        "Descrição detalhada da ação realizada*",
                        value=st.session_state.get(f"exec_action_desc_{notification.get('id', UI_TEXTS.text_na)}_v2tabs", ""),
                    ...
                    if st.session_state[action_choice_key] == "Concluir Minha Parte":
                        st.markdown("##### 📎 Evidências (obrigatório na conclusão)")
                        evidence_desc = st.text_area(
                            "Descrição das evidências*",
                            value=st.session_state.get(f"exec_evidence_desc_{notification.get('id', UI_TEXTS.text_na)}_v2tabs", ""),
                            placeholder="Ex: print de tela, protocolo, número de chamado, foto do reparo, e-mail enviado, etc.",
                            height=90,
                            key=f"exec_evidence_desc_widget_{notification.get('id', UI_TEXTS.text_na)}_v2tabs"
                        )
                        evidence_files = st.file_uploader(
                            "Anexar evidências (opcional, mas recomendado)",
                            accept_multiple_files=True,
                            key=f"exec_evidence_files_{notification.get('id', UI_TEXTS.text_na)}_v2tabs"
                        )

                    submitted = st.form_submit_button("💾 Salvar")

                if submitted:
                    choice = st.session_state.get(action_choice_key, UI_TEXTS.selectbox_default_acao_realizar)
                    desc = (action_description_state or "").strip()

                    if choice == UI_TEXTS.selectbox_default_acao_realizar:
                        st.error("Selecione uma opção válida em 'Qual ação deseja realizar?'.")
                        st.stop()
                    if not desc:
                        st.error("A descrição da ação é obrigatória.")
                        st.stop()

                    final_flag = (choice == "Concluir Minha Parte")

                    if final_flag:
                        if not (evidence_desc or "").strip():
                            st.error("Na conclusão, a descrição das evidências é obrigatória.")
                            st.stop()

                    # Persistir evidências (se houver) usando mesmo mecanismo de anexos da app
                    evidence_attachments = []
                    if final_flag and evidence_files:
                        for f in evidence_files:
                            saved = save_uploaded_file_to_disk(f, notification.get('id'))
                            if saved:
                                # também registrar no DB de attachments (padrão)
                                try:
                                    conn = get_db_connection()
                                    cur = conn.cursor()
                                    cur.execute("""
                                    ...
                                    # se falhar, não aborta a ação; apenas deixa sem vínculo no catálogo
                                    pass
                    ...
                    if ok:
                        ...
                        add_history_entry(
                            notification.get('id'),
                            "Ação registrada (Executor)" if not final_flag else "Conclusão registrada (Executor)",
                            user_username_logged_in,
                            desc if not final_flag else f"{desc}\n\nEvidências: {(evidence_desc or '').strip()}",
                        )

                        # Atualizar status se ainda não estiver em execução
                        if notification.get('status') in ['classificada', 'classificada_aguardando_execucao']:
                            update_notification(notification.get('id'), {"status": "em_execucao"})

                        st.success("✅ Ação salva com sucesso!")
                        st.rerun()
                    else:
                        st.error("❌ Não foi possível salvar a ação. Tente novamente.")

                st.markdown("---")

    with tab_pending:
        st.markdown("### ⏳ Pendentes de Execução")
        if not pending_notifications:
            st.success("✅ Nenhuma notificação pendente de execução atribuída a você no momento.")
        _render_collapsed_cards(pending_notifications, mode="pending")

    with tab_executed:
        st.markdown("### ✅ Executadas")
        st.caption("Aqui aparecem notificações onde **você já concluiu sua parte** (ação final registrada).")
        if not executed_notifications:
            st.info("Você ainda não concluiu nenhuma notificação.")
        _render_collapsed_cards(executed_notifications, mode="executed")
